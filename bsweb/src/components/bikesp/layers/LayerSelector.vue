<template>
  <label for="filter-select">{{$t(`COMMONS.layers_category.${category.category_name}`)}}:</label>
...
const options = computed(() => {
  ...
  { return {key: filter.filter_key, name: translateLabels(filter.filter_key), value: false} })
});

watch(value, (newValue) => {
  const selectedNames = new Set(newValue.map(item => item.key));
  const updatedLayers = options.value.map(option => ({
    key: option.key, 
...
onBeforeUnmount(() => {
  store.commit('bikesp/updateActiveLayers', options.value);
});
</script>
